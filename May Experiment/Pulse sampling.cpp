...
void setup() {
  ...
  while( ads.readADC_SingleEnded(NTCC_channel)> PID_Setpoint+10){
    ...
    Serial.print("NTCC: ");Serial.print(ads.readADC_SingleEnded(NTCC_channel));Serial.print(",");
...
double on_resistance = 0;
double off_resistance = 0;
double on_voltage = 0;
double off_voltage = 0;
...
void read_value(){
  ...
  // while(millis()-previous_pulse < 40){
  //   power_on = ads.readADC_SingleEnded(Sensor_channel); //9ms
  // } //35 ms
  ...
  // while(millis()-previous_pulse < 40){
    if(sht.readSample()){
      power_off = ads.readADC_SingleEnded(Sensor_channel);
      ...
      Serial.print(ads.readADC_SingleEnded(Sensor_channel));Serial.print(",");
      // on_voltage = ads.computeVolts(ads.readADC_SingleEnded(Sensor_channel));
      // on_resistance = ((load_resistance * input_voltage)/on_voltage) - load_resistance;
      off_voltage = ads.computeVolts(ads.readADC_SingleEnded(Sensor_channel));
      off_resistance = ((load_resistance * input_voltage)/off_voltage) - load_resistance;
      // Serial.print(on_resistance);Serial.print(",");
      Serial.print(off_resistance);Serial.print(",");
